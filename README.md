...
Agent는 MCP뿐 아니라 [Skill](https://github.com/anthropics/skills)을 활용하여 다양한 기능을 편리하게 구현할 수 있습니다. 여기에서는 [LangGraph](https://www.langchain.com/langgraph)에서 Agent skill을 활용하는 방법에 대해 설명합니다.

Web UI는 **FastAPI + React** (`application/server.py`, `application/web/`)이며, 추론은 같은 프로세스의 LangGraph Agent(`chat.run_langgraph_agent` → `langgraph_agent`)로 실행합니다. Agent 코드는 `application/`에 두어 `agentic-work/runtime_agent/langgraph`와 인터페이스를 맞춘 뒤 상용 배포에 재사용할 수 있습니다.
...
`config.json`의 AWS/KB/Memory 설정이 필요합니다. Agent는 로컬 LangGraph로 실행됩니다.
...
```mermaid
flowchart TB
  subgraph UI["FastAPI + React (server.py / web/)"]
    M[Task / Chat]
    SKUI[Skill / MCP 선택]
  end
...
| 기능 | 모듈 | 설명 |
|------|------|------|
| Web UI Chat | `chat.run_agent` → LangGraph | FastAPI SSE + React (`:8501`) |
| Agent | `langgraph_agent` + MCP/Skills | 동일 프로세스에서 실행 |
| RAG | `chat.run_rag_with_knowledge_base` | Bedrock Knowledge Base 검색 후 답변 |
| 이미지 분석 | `chat.summarize_image` | 멀티모달 이미지 요약 |
...
활성화할 스킬은 `config.json`의 `default_skills`(베이스)와 `plugin_skills`(플러그인별)에서 설정하며, Web UI에서도 선택할 수 있습니다.



## LangGraph에서 Skill의 구현

[chat.py](./application/chat.py)의 run_langgraph_agent는 사용자의 요청(query)를 Agent를 이용해 수행합니다. Web UI Task에서 선택한 MCP 서버 리스트로 mcp.json을 생성하여 server_params을 추출하고, MCP tool과 built-in tool을 추출하여 agent를 생성합니다. built-in tool에는 skill을 위한 get_skill_instructions과 execute_code, write_file, read_file 들이 있습니다.
...
Agent의 장기 기억은 [Amazon Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html)를 사용합니다. 대화에서 추출된 사용자 선호도·프로파일은 namespace(기본값: `/users/{actor_id}`)에 저장되며, **Memory MCP는 조회 전용**으로 동작합니다. 저장(`record`)·삭제(`delete`)는 MCP에서 제공하지 않습니다.

관련 코드:

- [mcp_server_memory.py](./application/mcp_server_memory.py): MCP 서버 (`recall_memory` 도구 노출)
- [mcp_memory.py](./application/mcp_memory.py): AgentCore Memory Retrieve/List/Get 호출
- [agentcore_memory.py](./application/agentcore_memory.py): memory id / actor / session / namespace 관리 및 대화 저장
- [mcp_config.py](./application/mcp_config.py): `memory` MCP 설정

#### 활성화

Web UI에서 MCP로 `memory`를 선택합니다. `Memory` 토글과도 함께 사용할 수 있습니다.

- **Memory MCP**: AgentCore 장기 기억(사용자 프로파일 포함)을 `recall_memory`로 조회
- **Memory Mode**: 워크스페이스 Markdown 메모리(`MEMORY.md`, `memory/*.md`)용 `memory_search` / `memory_get` 도구 활성화 및 대화 자동 저장

MCP 설정은 아래와 같습니다.

```python
# application/mcp_config.py
...
1. **사용자 컨텍스트 로드**  
   MCP 환경의 `user_id`로 [agentcore_memory.py](./application/agentcore_memory.py)의 `load_memory_variables()`를 호출해 `memory_id`, `actor_id`, `session_id`, `namespace`를 가져옵니다.

2. **검색 namespace 구성** (`retrieve` / `list`)  
   [mcp_memory.py](./application/mcp_memory.py)의 `get_search_namespaces()`가 아래를 합칩니다.
   - 사용자 기본 namespace (예: `/users/{actor_id}`)
   - 사용자 프로파일 namespace `/users/{actor_id}` (항상 포함)
   - Memory에 등록된 strategy들의 namespace (template에 `actorId` 등 반영)

3. **조회 및 병합**  
   각 namespace에 대해 AgentCore `retrieve_memory_records` / `list_memory_records`를 호출하고, 결과를 병합·중복 제거한 뒤 반환합니다. 사용자 프로파일(preference)과 일반 long-term memory를 한 번의 recall로 함께 볼 수 있습니다.

4. **대화 저장(쓰기)은 MCP 밖**  
   Memory Mode가 Enable일 때 앱이 대화 종료 후 `save_conversation_to_memory()`로 short-term event를 기록하면, AgentCore strategy(user preference extraction)가 long-term record로 추출합니다. Memory MCP는 이렇게 쌓인 record를 **읽어 쓰는 역할만** 수행합니다.
...
Web UI와 별개로 아래 명령어로 telegram bot을 준비합니다.
...
FastAPI + React SPA를 `uvicorn application.server:app --host 0.0.0.0 --port 8501`로 실행합니다. 로컬에서는 `./run_local.sh`를 사용하세요.
