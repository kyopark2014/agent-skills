...
description: >-
  drawing-devider로 자른 타일 DXF에서 벽을 검출하고 빨간색 WALL 레이어 DXF/PNG로
  저장합니다. 벽 검출, wall detect, wall DXF, 평면도 벽체, drawing-devider parts,
  floor_parts_index 요청 시 사용합니다.
---

# drawing-walldetector (벽 검출)

`drawing-devider`가 만든 **작은 타일 도면**에서 벽을 찾아 **빨간색**으로 표시한 DXF(및 검수용 PNG)를 생성한다.

## When to Use

- `drawing-devider` 산출물(`floors/<F>/parts/R*C*.dxf`)에서 벽을 뽑을 때
- 벽체 DXF / 빨간 벽 오버레이 / wall detect 요청 시
- 층 `floor_original`로 타일 위치를 확인한 뒤 타일 단위 벽 분석이 필요할 때

## Critical Rules

1. **입력은 devider 타일** — `$ARTIFACTS_DIR/<drawing_id>/floors/<F>/floor_parts_index.json`의 `parts[].files.dxf`만 사용한다. 원본 277MB DXF를 직접 돌리지 않는다.
2. **층 구조 먼저** — `$ARTIFACTS_DIR/<drawing_id>/floors/<F>/floor_original.png`로 전체 구조를 파악한 뒤 타일 위치를 확인한다.
3. **기존 walls 게이트** — `floors/<F>/walls/`가 **이미 있으면** 경로·요약을 보여 주고 **계속(덮어쓰기) / 중단**을 물은 뒤, 허락이 있을 때만 진행한다.
...
7. **산출 경로** — `$ARTIFACTS_DIR/<drawing_id>/floors/<F>/walls/` 아래에만 저장한다.
...
| 스크립트 | 용도 |
| --- | --- |
| `$WORKING_DIR/skills/drawing-walldetector/scripts/detect_walls_floor.py` | **한 층** 타일 + `floor_wall_original` (source=`floor_original`) |
| `$WORKING_DIR/skills/drawing-walldetector/scripts/detect_walls_tile.py` | 단일 타일 |
...
drawing-devider 산출물
  ...
  ↓
⓪ floors/<F>/walls/ 존재 여부 확인 → 있으면 계속/중단 확인
  ↓
① 파일럿 층: floor_original.png 확인
  ↓
② detect_walls_floor.py --floor <FIRST>   ← bash 1회
     → parts 타일 walls/ + floors/<F>/floor_wall_original.*
  ↓
③ 사용자 컨펌 (나머지 층)
  ↓
...
if [ -d "$ART/floors/$FLOOR/walls" ]; then
  echo "EXISTING: $ART/floors/$FLOOR/walls"
  ls "$ART/floors/$FLOOR/walls" 2>/dev/null | head -20
  # STOP: 허락 전 detect 금지
fi
```
...
- 두께 기본: 50–420 mm
- 최소 세그먼트 길이: 500 mm
- 계단 해칭(다수 평행선) · 작은 닫힌 사각형(기둥) · ARC/CIRCLE(문·설비) 제외
...
├── floors/<FLOOR>/
│   ├── floor_original.dxf / .png   # (devider) 층 전체 벽 입력 · 미리보기
│   ├── floor_parts_index.json      # (devider) 타일 목록
│   ├── parts/R0C0.dxf|.png         # (devider) 타일 입력
│   ├── floor_wall_original.dxf / .png / _meta.json  # 층 전체 (floor_original 기준)
│   └── walls/                      # (walldetector) 타일 산출
│       ├── walls_index.json
│       ├── R0C0_walls.dxf          # BASE 회색 + WALL 빨강
│       ├── R0C0_walls.png
│       └── R0C0_walls_meta.json
└── walls_all_index.json            # (선택) 다층 요약
```

타일 입력은 devider `parts/*.dxf`만. 층 전체 벽은 **`floor_original.dxf`** 만 사용한다.

---

## Decision Checklist

- [ ] `$ARTIFACTS_DIR/<drawing_id>/floors/<F>/` (devider 산출)가 있는가
- [ ] `floors/<F>/walls/`가 이미 있으면 **계속/중단**을 물었는가
...
| 증상 | 대응 |
|------|------|
| `floor_parts_index.json` 없음 | 먼저 `drawing-devider`로 해당 층 split |
...
- 층별 `$ARTIFACTS_DIR/<drawing_id>/floors/<F>/walls/` 경로
- 샘플 `*_walls.png` (벽=빨강)
- 파일럿 시: **나머지 층 진행 여부** / 완료 시: 층별 타일·벽 통계

## Related

- `drawing-devider` — 타일 분할·`floor_original.png` 선행 스킬
