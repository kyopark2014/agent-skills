...
def load_agentic_config() -> dict[str, Any]:
    ...
    # agentic-work: .../langgraph/skills/my-vaults/scripts
    # agent-skills: .../application/skills/my-vaults/scripts
    app_or_langgraph = here.parents[3]
    project_candidates = [here.parents[i] for i in (4, 5) if len(here.parents) > i]
    candidates.append(app_or_langgraph / "config.json")
...
def _project_name(cfg: Optional[dict[str, Any]] = None) -> str:
    ...
    return (
        (cfg.get("projectName") or os.environ.get("TASK_DB_PROJECT") or "agent-skills")
        .strip()
        or "agent-skills"
    )
...
def _session_signing_key() -> tuple[Optional[bytes], list[str]]:
    ...
    here = Path(__file__).resolve()
    # agent-skills: parents[3]=application; agentic-work: parents[5]=repo root
    key_paths = [
        Path.cwd() / "application" / "data" / ".session_signing_key",
        Path.cwd() / "data" / ".session_signing_key",
        here.parents[3] / "data" / ".session_signing_key",
    ]
    for i in (4, 5):
        if len(here.parents) > i:
            key_paths.append(here.parents[i] / "application" / "data" / ".session_signing_key")
