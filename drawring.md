...
- **H-Beam 기둥(중첩 정사각)은 벽** — 가구 사각 demote와 구분
...
#### 7) H-Beam 기둥이 회색으로 남음

| | 내용 |
|--|------|
| **증상** | 중첩 정사각(+ 내부 대시) 기둥이 BASE(회색). walldetector가 작은 닫힌 사각을 가구/기둥으로 일괄 제외한 결과 |
| **원인** | `is_non_wall_closed_box` / furniture demote가 기둥·가구를 구분하지 않음 |
| **대응** | (1) 동심 중첩 정사각(변≤1.2 m, 테두리 20–250 mm) = H-Beam (2) `promote_hbeam_columns` BASE→WALL (3) 가구 박스 demote에서 기둥 exclude (4) SKILL에 「기둥 = 벽」명시 |
